...
module Knapsack
    ...
    function lso!(kp :: KnapsackProblem , p :: Individual)
        bestFitness  = fitness(kp, p)
        copyInd = deepcopy(p)
        bestInd = deepcopy(p)
        for i = 2 : length(p.order)
            # Insert object i into the first position
            copyInd.order[1] = p.order[i]
            copyInd.order[2:i] = p.order[1:i-1]
            copyInd.order[i+1:end] = p.order[i+1:end]
            fv = fitness(kp, copyInd)
            if fv > bestFitness
                bestFitness = fv
                bestInd.order .= copyInd.order
                # println("The best fitness is now ", bestFitness)
            end
        end
        p.order .= bestInd.order
    end

    """ Advanced local search operator """
    function lso_advanced!(kp :: KnapsackProblem , p :: Individual)
        bestFitness  = fitness(kp, p)
        copyInd = deepcopy(p)
        bestInd = deepcopy(p)
        for i = 2 : length(p.order)
            for j = i+1 : length(p.order)
                # swap positions i and j
                copyInd.order[j] = p.order[i]
                copyInd.order[i] = p.order[j]
                fv = fitness(kp, copyInd)
                if fv > bestFitness
                    bestFitness = fv
                    bestInd.order .= copyInd.order
                ...
                # unswap positions i and j
                copyInd.order[i] = p.order[i]
                copyInd.order[j] = p.order[j]
            end
        end
        p.order .= bestInd.order
    end
    ...
    @time heurOrder = reverse(sortperm(kp.value ./ kp.weight))
    @time heurBest = Individual(heurOrder, 0.0)

    @time println("Heuristic objective value = ", fitness(kp, heurBest))

    p = Parameters( 200, 100, 5, 50 )
    @time evolutionaryAlgorithm(kp, p)
